import type { NativeContextParams, NativeCompletionParams, NativeCompletionResult, NativeTokenizeResult, NativeEmbeddingResult, NativeSessionLoadResult, NativeRerankResult, JinjaFormattedChatResult, ParallelStatus } from './types';
declare global {
    ...
    var llamaApplyLoraAdapters: (contextId: number, adapters: Array<{
        path: string;
        scaled?: number;
    }>) => Promise<void>;
    var llamaRemoveLoraAdapters: (contextId: number) => Promise<void>;
    var llamaGetLoadedLoraAdapters: (contextId: number) => Promise<Array<{
        path: string;
        scaled?: number;
    }>>;
    var llamaInitMultimodal: (contextId: number, params: {
        path: string;
        use_gpu?: boolean;
    ...
    var llamaGetMultimodalSupport: (contextId: number) => Promise<{
        vision: boolean;
        audio: boolean;
    }>;
    ...
    var llamaQueueCompletion: (contextId: number, params: NativeCompletionParams, onToken: (token: any, requestId: number) => void, onComplete: (result: any) => void) => Promise<{
        requestId: number;
    }>;
    ...
    var llamaSubscribeParallelStatus: (contextId: number, onStatus: (status: ParallelStatus) => void) => Promise<{
        subscriberId: number;
    }>;
    var llamaUnsubscribeParallelStatus: (contextId: number, subscriberId: number) => void;
}
//# sourceMappingURL=jsi.d.ts.map