...
exports.RNLLAMA_MTMD_DEFAULT_MEDIA_MARKER = exports.LlamaContext = exports.BuildInfo = void 0;
...
const installJsi = async () => {
  ...
  if (typeof global.llamaInitContext !== 'function') {
    const installed = await _NativeRNLlama.default.install();
...
const stripFilePrefix = path => path.replace(/^file:\/\//, '');
...
async function getBackendDevicesInfo() {
  await installJsi();
  const {
...
async function initLlama({
  ...
  if (Array.isArray(devices)) {
    ...
    if (_reactNative.Platform.OS === 'android' && devices.includes('HTP*')) {
...
const BuildInfo = exports.BuildInfo = {
  number: _version.BUILD_NUMBER,
